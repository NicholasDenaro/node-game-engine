...
export class SpiderRules implements GameRules {
    ...
    init(engine: EngineStateService, scene: Scene): void {
        ...
        for (let i = 0; i < 8; i++) {
            scene.addEntity(this.sorts[i] = new CardDeckEntity(`sort${i + 1}`, true, false, cardSize));
        }
    }
    ...
    canDropTo(dropTo: CardDeckEntity | CardStackEntity, cardLastFrom: CardDeckEntity | CardStackEntity, held: CardStackEntity | CardEntity | null): boolean {
        return this.getOption(this.ruleKeys.norules) || dropTo.count() == 0 || Number.parseInt(dropTo.peekCard().value) == Number.parseInt((held?.getEntities()[0] as CardEntity).value) + 1 || dropTo == cardLastFrom;
    }
    ...
    canPickUpStack(stack: CardStackEntity, pickupCount: number): boolean {
        ...
        for (let i = cards.length - pickupCount; i < cards.length - 1; i++) {
            if (Number.parseInt(cards[i].value) != Number.parseInt(cards[i + 1].value) + 1) {
                descending = false;
                break;
            }
        }
            ...
            for (let j = 0; j < cards.length; j++) {
                if (Number.parseInt(cards[j].value) == 13 && cards[j].isUp && cards.length - j >= 13) {
                    ...
                    for (let k = j + 1; k < cards.length; k++) {
                        if (!cards[j].isUp || Number.parseInt(cards[k].value) != Number.parseInt(cards[k - 1].value) - 1) {
                            descending = false;
                            break;
                        }
                    }
